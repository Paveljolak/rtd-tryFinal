...
apt-get update
apt-get -y install git rsync python3-pip python3-venv python3-git make

# create venv (PEP 668 safe)
python3 -m venv .venv
source .venv/bin/activate

# install python deps from required_packages.txt in repo root
...
for current_version in ${versions}; do
   ...
   languages="en `find docs/locales/ -mindepth 1 -maxdepth 1 -type d -exec basename '{}' \; || true`"
   for current_language in ${languages}; do
      ...
      # PDF #
      sphinx-build -b rinoh docs/ docs/_build/rinoh -D language="${current_language}" || true
      ...
      # EPUB #
      sphinx-build -b epub docs/ docs/_build/epub -D language="${current_language}" || true
...
# return to main branch (fallback to master if needed)
git checkout "${GITHUB_REF_NAME:-main}" || git checkout main || git checkout master
...
# add .nojekyll to the root so that github won't 404 on dirs starting with _
touch .nojekyll
